...
const wc = require("../outputs/DiplomaVerifier_js/witness_calculator.js");
const fs = require("fs");
...
async function main() {
  ...
  const merkleTreeData = JSON.parse(
    fs.readFileSync(__dirname + "/../data/input.json", "utf8")
  );
  
  // build witness calculator with webassembly
  const buffer = fs.readFileSync(__dirname + "/../outputs/DiplomaVerifier_js/DiplomaVerifier.wasm");
  const witnessCalculator = await wc(buffer);

  const witnessFile = await witnessCalculator.calculateWTNSBin(merkleTreeData, 0);

  const zKeyFile = fs.readFileSync(__dirname + "/../outputs/DiplomaVerifier_final.zkey");
